...
⚠️<span style=color:red>由于rocky官方yum源发生变更，在centos7上使用官方yum源安装会有部分包无法安装，因此采用离线包制作本地yum源方式安装</span>
...
⚠️<span style=color:red>本文使用本地源安装，不需要安装 `centos-release-openstack-rocky`</span>
...
**<span style=color:red>⚠️交互式与非交互式设置密码选择其中一种</span>**
...
**<span style=color:red>⚠️注意expires中是UTC时间，落后中国8个小时，我国是东八区，使用timedatectl查看时间及时区，默认过期时间1小时</span>**
...
**<span style=color:red>⚠️忽略此输出中的任何弃用消息</span>**
...
<span style=color:red>⚠️这一步一定要看执行后输出结果中size大小，如果为0则说明镜像上载有问题</span>
...
<span style=color:red>⚠️追加内容时要添加一行空行，否则格式会有错误(这里添加了两行空行，其中第一行是为了格式正确，第二行是为了格式规范，即标签与标签之间有一行空行)</span>
...
![iShot2021-11-04 17.09.24](https://gitee.com/pptfz/picgo-images/raw/master/img/iShot2021-11-04 17.09.24.png)
...
**⚠️<span style=color:red>如果是在虚拟机中，需要添加一块数据盘，使用命令 `echo "- - -" >/sys/class/scsi_host/host0/scan` 实现热加载（不一定为host0，也可能是其他数字，比如1、2、3等）</span>**
...
**<span style=color:red>⚠️在每个对象节点上执行以下步骤</span>**
...
**<span style=color:red>在控制节点和两个对象存储节点执行</span>**
...
**<span style=color:red>两个对象存储节点执行</span>**
