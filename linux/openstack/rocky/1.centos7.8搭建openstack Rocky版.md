...
3.在10.0.0.51安装nginx，因为是采用http方式
yum -y install nginx
...
**<span style=color:red>到此，控制节点和计算、存储、对象节点操作完成！！！</span>**
...
### 2.3.5 启动rabbitmq一个插件，启动之后会监听tcp/15672，是一个web管理界面，默认用户名和密码都是``guest``
...
### 4.3.1 创建``glance``用户，密码设置为``GLANCE_PASS``
...
### 4.3.2 将`admin`角色添加到`glance`用户和 `service`项目
...
### 4.4.2 编辑``/etc/glance/glance-api.conf``文件并完成以下操作
...
### 4.4.3 编辑``/etc/glance/glance-registry.conf``文件并完成以下操作
...
| created_at       | 2020-05-25T10:49:34Z                                                                                                                                                                       |
| disk_format      | qcow2                                                                                                                                                                                      |
| file             | /v2/images/94c96aab-d0b3-4340-835c-9a97108d0554/file                                                                                                                                       |
| id               | 94c96aab-d0b3-4340-835c-9a97108d0554                                                                                                                                                       |
| min_disk         | 0                                                                                                                                                                                          |
| min_ram          | 0                                                                                                                                                                                          |
| name             | cirros                                                                                                                                                                                     |
| owner            | 108d3fecb61840e3818f694c69c3ec4a                                                                                                                                                           |
...
| status           | active                                                                                                                                                                                     |
| tags             |                                                                                                                                                                                            |
| updated_at       | 2020-05-25T10:49:34Z                                                                                                                                                                       |
| virtual_size     | None                                                                                                                                                                                       |
| visibility       | public                                                                                                                                                                                     |
+------------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
```
...
## 5.1 创建nova、nova_api、nova_cell0、placement数据库并授权
...
### 5.3.1 创建``nova``用户，密码设置为``NOVA_PASS``
...
### 5.3.5 创建展示位置服务用户``PLACEMENT``，密码设置为``PLACEMENT_PASS``
...
### 5.3.6 使用管理员角色将Placement用户添加到服务项目中
...
### 5.3.7 在服务目录中创建``Placement API``条目
...
### 5.4.2 编辑``/etc/nova/nova.conf``文件并完成以下操作
...
6b2990f21ec6a41a4c33f2a25686a44e  /etc/nova/nova.conf
```



### 5.4.3 由于[包装错误](https://bugzilla.redhat.com/show_bug.cgi?id=1430540)，您必须通过将以下配置添加到来启用对Placement API的访问 `/etc/httpd/conf.d/00-nova-placement-api.conf`

```python
1.备份文件并追加内容
\cp /etc/httpd/conf.d/00-nova-placement-api.conf{,.bak}

2.注意要添加一行空行，否则格式会有错误(这里添加了两行空行，其中第一行是为了格式正确，第二行是为了格式规范，即标签与标签之间有一行空行)
...
### 5.4.4 启动Compute服务并将其配置为在系统引导时启动
...
### 5.5.2 编辑``/etc/nova/nova.conf``文件并完成以下操作
...
MD5值
md5sum /etc/nova/nova.conf
ab7abb82433ee2a9f2381aa10018590a  /etc/nova/nova.conf
```



### 5.5.3 确定您的计算节点是否支持虚拟机的硬件加速
...
### 5.5.4 启动Compute服务及其依赖项，并将它们配置为在系统引导时自动启动
...
### 5.6.1 获取管理员凭据以获取对仅管理员CLI命令的访问权限
...
+----+--------------+----------+------+---------+-------+----------------------------+
|  9 | nova-compute | compute1 | nova | enabled | up    | 2020-05-25T23:37:57.000000 |
+----+--------------+----------+------+---------+-------+----------------------------+
```
...
### 6.3.1 创建``neutron``用户，密码设置为``NEUTRON_PASS``
...
### 6.3.2 将``admin``角色添加到``neutron``用户
...
### 6.11.1 网络服务初始化脚本需要`/etc/neutron/plugin.ini`指向ML2插件配置文件的符号链接 `/etc/neutron/plugins/ml2/ml2_conf.ini`。如果此符号链接不存在，请使用以下命令创建它
...
# 七、控制节点horizon web界面Dashboard安装
...
**⚠️因为有一些内容是删除之后粘贴的，所以一些行数并不是很准确，但是行数误差不超过3行**
...
**采用cat EOF方式可能会有格式问题，因为文件内容太多了，这里必须使用vi打开文件然后复制粘贴内容，不能使用vim会有格式错误**
...
## 7.3 修改配置文件，否则后续访问dashboard会报500错误

**`/etc/httpd/conf.d/openstack-dashboard.conf`如果不包含以下内容，请添加以下行 **
...
![iShot2020-05-2620.16.57](https://gitee.com/pptfz/picgo-images/raw/master/img/iShot2020-05-2620.16.57.png)



**登陆后首界面**

![iShot2020-05-2620.18.46](https://gitee.com/pptfz/picgo-images/raw/master/img/iShot2020-05-2620.18.46.png)



# 八、控制节点和存储节点块存储服务cinder安装
...
### 8.3.1 创建``cinder``用户，密码设置为``CIDNER_PASS``
...
### 8.3.3 创建`cinderv2`和`cinderv3`服务实体
...
### 8.5.2 编辑`/etc/cinder/cinder.conf`文件并完成以下操作
...
7fd4cf1f205df40cb8f01a280110988e  /etc/cinder/cinder.conf
```
...
MD5值
md5sum /etc/nova/nova.conf
a8f15dce52b4c6a2ae8f4768a20f66a5  /etc/nova/nova.conf
```
...
### 8.8.2 启动LVM元数据服务，并将其配置为在系统引导时启动
...
### 8.8.3 创建LVM物理卷`/dev/sdb`

**⚠️如果是在虚拟机中，需要添加一块数据盘，使用命令``echo "- - -" >/sys/class/scsi_host/host0/scan``实现热加载（不一定为host0，也可能是其他数字，比如1、2、3等）**
...
**如果存储节点在操作系统磁盘上使用LVM，则还必须将关联的设备添加到过滤器中。例如，如果`/dev/sda`设备包含操作系统：**

```python
filter = [ "a/sda/", "a/sdb/", "r/.*/"]
```

**同样，如果您的计算节点在操作系统磁盘上使用LVM，则还必须`/etc/lvm/lvm.conf`在这些节点上的文件中修改过滤器， 使其仅包括操作系统磁盘。例如，如果`/dev/sda` 设备包含操作系统：**
...
**因为使用的虚拟机在安装的时候是采用的lvm，所以配置文件中应当把系统盘``/dev/sda``也添加**
...
f14b4b4075998bf20d1186089cfcdc40  /etc/lvm/lvm.conf
```
...
### 8.9.2 编辑`/etc/cinder/cinder.conf`文件并完成以下操作

**⚠️在配置文件中的[DEFAULT]区域，默认官方定义的后端lvm卷名称就是lvm，这个名称是任意的，并且DEFAULT区域的enabled_backends定义的名称是有单独的一个区域，例如**
...
MD5值
md5sum /etc/cinder/cinder.conf
07cc203c56289c1ed19d6f3daba7ec32  /etc/cinder/cinder.conf
```



### 8.9.3 启动块存储卷服务及其相关性，并将其配置为在系统启动时启动
...
**<span style=color:red>到此，控制节点和块存储节点块存储服务cinder安装完成！！！</span>**




# 九、控制节点和对象节点对象存储服务swift安装
...
### 9.1.2 创建`swift`用户，密码设置为``SWIFT_PASS``
...
### 9.3.3 编辑`/etc/swift/proxy-server.conf`文件并完成以下操作

**<span style=color:red>⚠️⚠️⚠️这里官方文档有坑，q版之后就不用35357端口了，但是文档中还是写着``auth_url = http://controller:35357``，这里应该改成5000端口，否则后续验证swift会报错500</span>**
...
MD5值
md5sum /etc/swift/proxy-server.conf
007b21d8dbe36156ff7fffff53e88f5a  /etc/swift/proxy-server.conf
```



<h3 style=color:red>安装和配置对象节点(51,52)</h3>
...
### 9.4.2 将`/dev/sdb`和`/dev/sdc`设备格式化为XFS
...
### 9.4.4 编辑`/etc/fstab`文件并向其中添加以下内容
...
### 9.4.6 创建或编辑`/etc/rsyncd.conf`文件以包含以下内容
...
Md5值
md5sum /etc/rsyncd.conf 
1871754e46ba67008183bad11758e7d5  /etc/rsyncd.conf
...
MD5值
md5sum /etc/rsyncd.conf 
064874b61f431372377b6b4222201872  /etc/rsyncd.conf
```
...
**<span style=color:red>⚠️在每个对象节点(51,52)上执行以下步骤</span>**
...
### 9.5.2 从对象存储源存储库中获取计费，容器和对象服务配置文件
...
### 9.5.3 编辑`/etc/swift/account-server.conf`文件并完成以下操作
...
MD5值
md5sum /etc/swift/account-server.conf
201ae87b1a85fb5d27a493b08bad6aab  /etc/swift/account-server.conf

#10.0.0.52
\cp /etc/swift/account-server.conf{,.bak}
grep '^[a-Z\[]' /etc/swift/account-server.conf.bak > /etc/swift/account-server.conf
openstack-config --set /etc/swift/account-server.conf DEFAULT bind_ip 10.0.0.52
openstack-config --set /etc/swift/account-server.conf DEFAULT bind_port 6202
openstack-config --set /etc/swift/account-server.conf DEFAULT user swift
openstack-config --set /etc/swift/account-server.conf DEFAULT swift_dir /etc/swift
openstack-config --set /etc/swift/account-server.conf DEFAULT devices /srv/node
...
MD5值
md5sum /etc/swift/account-server.conf
4896e265fa7f08cc0b3f71e9c0255647  /etc/swift/account-server.conf
```



### 9.5.4 编辑`/etc/swift/container-server.conf`文件并完成以下操作
...
MD5值
md5sum /etc/swift/container-server.conf
8da5dc93bb2b4dcaf34dddf6244c5a50  /etc/swift/container-server.conf


#10.0.0.52
\cp /etc/swift/container-server.conf{,.bak}
grep '^[a-Z\[]' /etc/swift/container-server.conf.bak > /etc/swift/container-server.conf
openstack-config --set /etc/swift/container-server.conf DEFAULT bind_ip 10.0.0.52
openstack-config --set /etc/swift/container-server.conf DEFAULT bind_port 6201
openstack-config --set /etc/swift/container-server.conf DEFAULT user swift
openstack-config --set /etc/swift/container-server.conf DEFAULT swift_dir /etc/swift
openstack-config --set /etc/swift/container-server.conf DEFAULT devices /srv/node
...
MD5值
md5sum /etc/swift/container-server.conf
027623ff6b9d87b218703efbdffe73ca  /etc/swift/container-server.conf
```



### 9.5.5 编辑`/etc/swift/object-server.conf`文件并完成以下操作
...
MD5值
md5sum /etc/swift/object-server.conf
a3decd3b7143c7255f732d5b8872f449  /etc/swift/object-server.conf

#10.0.0.52
\cp /etc/swift/object-server.conf{,.bak}
grep '^[a-Z\[]' /etc/swift/object-server.conf.bak > /etc/swift/object-server.conf
openstack-config --set /etc/swift/object-server.conf DEFAULT bind_ip 10.0.0.52
openstack-config --set /etc/swift/object-server.conf DEFAULT bind_port 6200
openstack-config --set /etc/swift/object-server.conf DEFAULT user swift
openstack-config --set /etc/swift/object-server.conf DEFAULT swift_dir /etc/swift
openstack-config --set /etc/swift/object-server.conf DEFAULT devices /srv/node
...
MD5值
md5sum /etc/swift/object-server.conf
a8c6c63f0d34ea0b9edc1207f7326aa7  /etc/swift/object-server.conf
```



### 9.5.6 确保对安装点目录结构拥有适当的所有权
...
### 9.5.7 创建`recon`目录并确保对其拥有适当的所有权
...
**⚠️<span style=color:red>在控制节点执行以下步骤</span>**
...
```python
$ swift-ring-builder account.builder
account.builder, build version 4, id 7fcd3aae59374c538cc6c368804c5102
...
### 9.8.2 创建基础`container.builder`文件
...
```python
$ swift-ring-builder container.builder
container.builder, build version 4, id 72f47fcf245841f19ba211e19f3752f4
1024 partitions, 3.000000 replicas, 1 regions, 2 zones, 4 devices, 100.00 balance, 0.00 dispersion
The minimum number of hours before a partition can be reassigned is 1 (0:00:00 remaining)
The overload factor is 0.00% (0.000000)
...
```python
$ swift-ring-builder object.builder
object.builder, build version 4, id 58129d88cd59455ea3c53172d05a5399
1024 partitions, 3.000000 replicas, 1 regions, 2 zones, 4 devices, 100.00 balance, 0.00 dispersion
The minimum number of hours before a partition can be reassigned is 1 (0:00:00 remaining)
The overload factor is 0.00% (0.000000)
...
```python
scp /etc/swift/{account.ring.gz,container.ring.gz,object.ring.gz} object1:/etc/swift
  
scp /etc/swift/{account.ring.gz,container.ring.gz,object.ring.gz} object2:/etc/swift
```
...
### 9.11.1 `/etc/swift/swift.conf`从对象存储源存储库中获取文件

**<span style=color:red>⚠️这一步在控制节点执行</span>**
...
### 9.11.2 编辑`/etc/swift/swift.conf`文件并完成以下操作
...
MD5值
md5sum /etc/swift/swift.conf
7fdc00e448c50b77fc703debf654d906  /etc/swift/swift.conf
```



### 9.11.3 将`swift.conf`文件复制到`/etc/swift`每个存储节点以及运行代理服务的所有其他节点上的目录中
...
### 9.11.4 在所有节点上，确保对配置目录拥有适当的所有权

**<span style=color:red>在控制节点和两个对象存储节点(51、52)执行</span>**
...
### 9.11.5 在控制器节点和运行代理服务的任何其他节点上，启动对象存储代理服务及其相关性，并将它们配置为在系统启动时启动
...
### 9.11.6 在存储节点上，启动对象存储服务并将其配置为在系统引导时启动

**<span style=color:red>两个对象存储节点(51、52)执行</span>**
...
**<span style=color:red>在控制器上执行以下步骤</span>**
...
**<span style=color:red>⚠️⚠️⚠️这里不知道是什么原因(也肯能是我自己哪里出错了)，如果按照官方文档中加载demo凭证，那么在执行``swift stat``命令时会报错403权限拒绝，所以这里加载了admin凭证</span>**
...
**<span style=color:red>到此，对象存储服务swift安装完成</span>**
...
**<span style=color:red>在块存储节点上执行以下步骤</span>**
...
## 10.2 编辑`/etc/cinder/cinder.conf`文件并完成以下操作
...
**<span style=color:red>替换`SWIFT_URL`为对象存储服务的URL。可以通过显示对象库API端点来找到URL，在控制节点上执行命令``openstack catalog show object-store``</span>**
...
**<span style=color:red>到此块存储节点块存储备份服务安装完成！！！</span>**
