...
<h3 span style=color:red>&emsp;使用&emsp;&emsp;ansible-doc 模块名&emsp;&emsp;查看模块使用帮助</h3>



**Ansible注意事项->提示颜色信息说明**

- <span style=color:#FFE351>淡黄色：对远程节点进行相应修改</span>
...
## 1.1 command模块 不支持管道 不支持通配符 默认模块
...
## 1.2 shell模块 支持管道 支持通配符

```python
[root@ansible ~]# ansible all -m shell -a "ifconfig|grep eth0" -f 50
...
## 1.3 raw模块 支持管道 支持通配符

```python
[root@ansible ~]# ansible all -m raw -a "ifconfig" 
```
...
[root@ansible ~]# ansible web01 -m script -a "/server/scripts/yum.sh"
```
...
```python
[root@ansible ~]# ansible all -m yum -a "name=httpd state=installed"
name			#指定要安装的软件包名称
state			#指定使用yum的方法
...
```python
//推送文件模块
[root@ansible ~]# ansible all -m copy -a "src=/etc/hosts dest=/tmp/test.txt"

//在推送覆盖远程端文件前，对远端已有文件进行备份，按照时间信息备份
[root@ansible ~]# ansible all -m copy -a "src=/etc/hosts dest=/tmp/test.txt backup=yes"
...
[root@ansible ~]# ansible all -m copy -a "content='abc' dest=/tmp/abc"
...
## 12.1 mysql_user管理用户模块

```python
[root@ansible ~]# ansible mysql -m mysql_user -a 'login_host=localhost login_password=abc123 login_user=root name=abc password=abc123 priv=zabbix.*:ALL state=present'
...
## 12.2 mysql_db管理数据库模块

```python
[root@ansible ~]# ansible mysql -m mysql_db -a 'login_host=localhost login_password=abc123 login_user=root name=hehe state=present'
