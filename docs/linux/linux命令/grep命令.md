...
| `*`      | 前边的字符连续出现0次或多次       | `*love`        | `love` 前边有0个或多个连续出现的字符                         |              |
| `.`      | 匹配除换行外的多个字符            | `l..e`         | 字母 `l` 和 `e` 之间有2个任意字符                            |              |
| `[]`     | 匹配中括号内的任意字符            | `[lL]ove`      | 匹配 `love` 或 `Love`                                        |              |
| `[^]`    | 匹配不在中括号内的任意字符        | `[^a-km-z]ove` | 匹配包含 `ove` ，但 `ove` 之前的那个字符不在 `a` 至 `k` 或 `m` 至 `z` 之间的字符 |              |
| `()`     | 分组，主要做反向引用              | `(a)\1`        | `(a)`: 这个部分会捕获字符 `a`，并将其存储在第一个捕获组中。`\1`: 这是一个反向引用，指的是第一个捕获组中的内容。也就是说，`\1` 会匹配捕获组中已经匹配到的内容（在这里就是字符 `a`）因此，`(a)\1` 这个正则表达式会匹配两个连续的 `a`，即 `aa`![iShot_2024-08-23_19.38.09](https://gitea.pptfz.cn/pptfz/picgo-images/raw/branch/master/img/iShot_2024-08-23_19.38.09.png) |              |
| `&`      | 保存查找串以便在替换中引用        | `s/love/&`     | `&` 代表查找串，将 `love` 替换为 `love`，也就是不对文本进行任何改变 |              |
| `<`      | 词首定位符                        | ``             |                                                              |              |
| `>`      | 词尾定位符                        | ``             |                                                              |              |
| `x{m}`   | 连续 `m` 个 `x`                   | ``             |                                                              |              |
| `x{m,}`  | 至少 `m` 个 `x`                   | ``             |                                                              |              |
| `x{m,n}` | 至少 `m` 个 `x` ，至多 `n` 个 `x` | ``             |                                                              |              |
...
![iShot2020-10-15 19.43.51](https://gitea.pptfz.cn/pptfz/picgo-images/raw/branch/master/img/iShot2020-10-15%2019.43.51.png)
