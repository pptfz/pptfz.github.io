...
nvm是一个 [node.js](https://nodejs.org/zh-cn)  的版本管理工具，使用nvm可以方便的通过命令行来切换不同的node版本
...
修改
	`https://github.com/${NVM_GITHUB_REPO}.git`

修改为
	`https://gh.con.sh/https://github.com/${NVM_GITHUB_REPO}.git`
...
按照提示需要将如下内容加入到本机的相应文件中，我这里使用的是 `zsh` ，因此是用户家目录下的 `.zshrc`
