...
### 总结对比：SSH Agent 与 macOS Keychain

| 特性                       | SSH Agent                                        | macOS Keychain                                        |
| -------------------------- | ------------------------------------------------ | ----------------------------------------------------- |
| **平台支持**               | 所有 Unix/Linux/macOS 平台                       | 仅 macOS                                              |
| **是否跨终端持久**         | ❌ 否，关闭终端或重启后需重新 `ssh-add`           | ✅ 是，重启后自动记住密码                              |
| **是否需要手动启动 agent** | ✅ 是，通常用 `eval "$(ssh-agent -s)"` 启动       | ❌ 否，Keychain 系统服务常驻                           |
| **密码保存方式**           | 缓存在当前终端会话中，重启失效                   | 保存在系统级 Keychain，永久有效                       |
| **添加密钥命令**           | `ssh-add ~/.ssh/id_rsa`                          | `/usr/bin/ssh-add --apple-use-keychain ~/.ssh/id_rsa` |
| **配置方式**               | `.bashrc` / `.zshrc` 中配置 agent 启动与添加命令 | `.ssh/config` 中加 `UseKeychain yes`                  |
| **安全性**                 | 安全（临时缓存，自动过期）                       | 安全（加密存储在系统钥匙串）                          |
| **适合场景**               | 临时会话、Linux 环境、服务器部署                 | 常用开发机（Mac），长期记住密钥密码                   |
